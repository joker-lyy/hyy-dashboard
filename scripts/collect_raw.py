...
def beijing_today():
    """Actions runner 默认使用 UTC；业务月份边界必须按北京时间计算。"""
...
def slim_zj(rows):
    ...
    fix46：补齐 rid/sid/tid 字段，前端弹窗"查看详情"链接靠它们构造自检报告 URL。
    """
    out = []
    for r in rows or []:
        ...
        out.append({
            ...
            # fix46：补 rid/sid/tid（与 slim_cg/slim_sp 对齐）
            "rid": str(r.get("reportId") or ""),
            "sid": str(r.get("signId") or ""),
            "tid": str(r.get("templateId") or ""),
        })
...
def fetch_month(api, cfg, month, fetched_at):
    ...
    for pos_name, org_name in cfg.TARGET_POSITIONS:
        try:
            ...
            # 6) 组织树叶子节点（门店总数）
            # fix26：必须传 root_name=org_name。
            # 否则 fix21 切到 888 总部 token 后「新店运营组/加盟营运组/新店筹建组」
            # 这三个走策略二的组，会拿到整棵树 42 个叶子而不是各组自己的叶子，
            # 导致 data.json 里每张卡的 regions 都变成 42 个全树标签。
            try:
