...
# fix208（2026-09-25）：平台 9/21 撤销「新店运营组」组织节点后，切组取数失败，
# 导致区域排名 rankings_*.json 每天全部生成失败（线上停在 9/19）。
# 按 fix202 虚拟桶口径：新店运营组 = 刘浩区域 + 陈晓君区域 + 开业督导组，
# 三个子节点仍在组织树中（刘浩/陈晓君现挂加盟营运组下、开业督导组挂加盟服务部下），
# 区域排名按子组织逐个切组取数，输出 position 仍统一标「新店运营组」。
# 注意与 update_store_counts 的 XY_REGION_NAMES（含范鑫区域占位）不同：
# 这里只列有叶子节点的三个，与撤组前 rankings 的 3 行输出对齐。
XY_VIRTUAL_SUBORGS = ("刘浩区域", "陈晓君区域", "开业督导组")
