...
function aggregateRegular(months, start, end) {
  ...
  for (const [posLabel, leaves] of Object.entries(leafMap)) {
    ...
    // 岗位汇总
    // 「门店数」用 posStores.length（当前区间内至少出过 1 次报告的独立门店数），
    // 不再用 leaves.currentStoreCount 之和（那是组织树全部门店数，会把未巡检门店也计进来，
    // 导致本月只跑了几家却显示 8/46/341 的"假大数"）。
    const posStores = allStores.filter(s => s.position === posLabel);
    const posScored = posStores.filter(s => s.score > 0);
    const storeCount = posStores.length;
    // 「覆盖门店总数」仍是组织树之和（与「门店数」含义不同：一个看应覆盖，一个看已巡检）
